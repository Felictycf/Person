    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:orderfmor2:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:orderfmor2:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:orderfmor2:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:orderfmor2:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="orderfmor2List" @selection-change="handleSelectionChange">
...
<script>
import { listOrderfmor2, getOrderfmor2, delOrderfmor2, addOrderfmor2, updateOrderfmor2 } from "@/api/system/orderfmor2";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOrderfmor2(this.queryParams).then(response => {
        this.orderfmor2List = response.rows;
    ...
    handleUpdate(row) {
      ...
      getOrderfmor2(onum).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.onum != null) {
            updateOrderfmor2(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOrderfmor2(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/orderfmor2/export', {
        ...this.queryParams
      }, `orderfmor2_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
