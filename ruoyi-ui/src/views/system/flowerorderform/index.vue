<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="订单编号" prop="onum">
        <el-input
      ...
      <el-form-item label="花柜号" prop="canzuohao">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:flowerorderform:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:flowerorderform:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:flowerorderform:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:flowerorderform:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="flowerorderformList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="订单编号" align="center" prop="onum" />
      <el-table-column label="人数" align="center" prop="opeople" />
      <el-table-column label="花柜号" align="center" prop="canzuohao" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="花柜号" prop="canzuohao">
          <el-input v-model="form.canzuohao" placeholder="请输入花柜号" />
        </el-form-item>
        <el-form-item label="手机号码" prop="cotel">
...
<script>
import { listFlowerorderform, getFlowerorderform, delFlowerorderform, addFlowerorderform, updateFlowerorderform } from "@/api/system/flowerorderform";

export default {
  name: "Flowerorderform",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFlowerorderform(this.queryParams).then(response => {
        this.flowerorderformList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getFlowerorderform(onum).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.onum != null) {
            updateFlowerorderform(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFlowerorderform(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除鲜花订单信息编号为"' + onums + '"的数据项？').then(function() {
        return delFlowerorderform(onums);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/flowerorderform/export', {
        ...this.queryParams
      }, `flowerorderform_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
